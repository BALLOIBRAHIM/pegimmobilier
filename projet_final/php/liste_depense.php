...
$reqc="SELECT*FROM depense WHERE depense.PROPRIETAIRE_P_num=?";
$prepare=$cnx->prepare($reqc);
$execute=$prepare->execute(array($proprietaire));
$cdepense=$prepare->FETCH(PDO::FETCH_ASSOC);
...
<body style=" background-color:rgb(166, 167, 164);" >
	...
	<h2 style="margin-left:330px;margin-top :30px;margin-bottom:30px; color:#ffffff;font-size:4rem;">Liste des dépenses</h2>
...
<div class="container">
		...
		<form method="POST" action="">
				...
				<tr>
                    ...
                    $bien=recherche_unique("bien","B_num",$cdepense['BEIN_B_num']);
                    ?>
					<td width="300px" height="90px">
					<?php echo $cdepense['D_date'];?>
					</td>
					<td width="300px" height="90px">
					<?php echo $cdepense['D_montant'];?>
					</td>
					<td width="300px" height="90px">
					...
					<table border="1" width="100px" cellspacing="0" cellpadding="0"><tr><td>
						<a href="supprimer_depense.php?  x=<?php echo $cdepense['D_num']?>" >
						 ...
						 <a href="modifier_depense.php?  x=<?php echo $cdepense['D_num'];?>" >
				...
				<?php }while($cdepense=$prepare->FETCH(PDO::FETCH_ASSOC));
				?>
			</table>
				</form>
				  </div>	
